...
type
  TfrmMain = class(TForm)
    ...
    mmiKlaster: TMenuItem;
    N4: TMenuItem;
    mmiTransparent: TMenuItem;
    mmiOnlyCenter: TMenuItem;
    mmiDB: TMenuItem;
    mmitest: TMenuItem;
    procedure GLInit;
    procedure ListPrep;
    ...
    procedure mmiKlasterClick(Sender: TObject);
    procedure mmiTransparentClick(Sender: TObject);
    procedure mmiOnlyCenterClick(Sender: TObject);
    procedure mmiDBClick(Sender: TObject);
...
uses  ParObject,unSets, unPlane, unPalletka, unDB;
  ...
  For i := 1 to objectcount do begin
     ...
     // glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);
      ...
      glMaterialfv(GL_FRONT, GL_AMBIENT_AND_DIFFUSE, @objects[i].color);
              ...
              //GL_DST_ALPHA
              //2  GL_ONE_MINUS_SRC_COLOR
//      glMaterialfv(GL_back, GL_EMISSION, @objects[i].color);
      glPushMatrix;
...
begin
        ...
        begin
           ...
           eLn_y.Enabled:=true;
           eLn_z.Enabled:=true;
           eSetColMol.Enabled:=true;
           if unSets.Canceled then exit;


           Control.RunPlane(radius,progress,mmiKlaster.Checked);
        end;
end;
//=============================
                ...
                for yi:=0 to ln_y-1 do
                 ...
                 if Contrl.grid[xi,yi,zi] <> nil then
                    With objects [objectcount + 1] do
                      begin
                      if mmiKlaster.Checked then
                      begin
                       if Contrl.grid[xi,yi,zi].inklaster then
                        begin
...
var     xi,yi,zi,ii:integer;
        ...
        resKraft,resOmega,p1,p2,p3,gamma,alfa,c,a,kkraft,RdoDot:real;
begin
       colsetMol:=0;
...
//          showmessage('in fori 2 xi='+inttostr(xi)+'; yi='+inttostr(yi));
                     ...
                     if x > a2 then
                     begin
                        RdoDot:=sqrt(sqr(x-Contrl.arr_plane[NumPlane,xi,yi].x)+
                                    sqr(d)+
                                    sqr(b));

                        omega:=pi*sqr(Contrl.arr_plane[NumPlane,xi,yi].radius)*cos(fi2)/sqr(RDoDot);
                     end else
                     begin
                        RdoDot:=sqrt(sqr(x-Contrl.arr_plane[NumPlane,xi,yi].x)+
                                    sqr(d)+
                        ...
                        omega:=pi*sqr(Contrl.arr_plane[NumPlane,xi,yi].radius)*cos(fi1)/sqr(RDoDot);
...
begin
        ...
        begin
              ...
              Control.RunRandomize(Radius,
                                   progress,over,ern,statusBar,colmol);
              eColMol.Text:=IntToStr(Control.colMol);
              eColMol.Update;
              eOtkazov.Text:=IntToStr(Control.otkazov);
              eOtkazov.Update;
...
begin
 ...
 mmiKlaster.Enabled:=Control.grid_exists;
end;
...
procedure TfrmMain.mmiKlasterClick(Sender: TObject);
begin
 if not mmiKlaster.Checked then
  mmiKlaster.Checked:=true else
  mmiKlaster.Checked:=false;
end;

procedure TfrmMain.mmiTransparentClick(Sender: TObject);
begin
  if mmiTransparent.Checked then
  begin
    TransparentMol:=false;
    mmiTransparent.Checked:=false;
  end else
  begin
    TransparentMol:=true;
    mmiTransparent.Checked:=true;
  ENd;
      
end;

procedure TfrmMain.mmiOnlyCenterClick(Sender: TObject);
begin
  if mmiOnlyCenter.Checked then
  begin
     mmiOnlyCenter.Checked:=false;
     onlyCenter:=false;
  end else
  begin
     mmiOnlyCenter.Checked:=true;
     onlyCenter:=true;
  end;
end;

procedure TfrmMain.mmiDBClick(Sender: TObject);
begin
   frmDB.show;
end;

procedure TfrmMain.mmitestClick(Sender: TObject);
const dotmol=0.3;
var     xi,yi,zi,ii:integer;
        ColSetMol:Integer;      //col установленныx молекул
begin
         ColSetMol:=0;
         ObjectCount:=0;
         if not Control.grid_exists then exit;
         flgMouseMove := False;
         with frmSets do
              for ii:=0 to control.glubina do
{              for zi:=0 to ln_z-1 do
                for yi:=0 to ln_y-1 do
                 for xi:=0 to ln_x-1 do}
//                 control.arrInklaster[ii].x
                 begin
                 if Control.grid[control.arrInklaster[ii].x,control.arrInklaster[ii].y,control.arrInklaster[ii].z] <> nil then
                    With objects [objectcount + 1] do
                      begin
                      ...
                      begin
                          ...
                          x :=Control.grid[control.arrInklaster[ii].x,control.arrInklaster[ii].y,control.arrInklaster[ii].z].x-Control.grid[control.arrInklaster[ii].x,control.arrInklaster[ii].y,control.arrInklaster[ii].z].radius;

                          y :=Control.grid[control.arrInklaster[ii].x,control.arrInklaster[ii].y,control.arrInklaster[ii].z].y-Control.grid[control.arrInklaster[ii].x,control.arrInklaster[ii].y,control.arrInklaster[ii].z].radius;

                          z :=Control.grid[control.arrInklaster[ii].x,control.arrInklaster[ii].y,control.arrInklaster[ii].z].z-Control.grid[control.arrInklaster[ii].x,control.arrInklaster[ii].y,control.arrInklaster[ii].z].radius;

                      showmessage('x='+floatToStr(x)+#13#10+
                              'y='+floatToStr(y)+#13#10+
                              'z='+floatToStr(z));
                          begin
                            l := Control.grid[control.arrInklaster[ii].x,control.arrInklaster[ii].y,control.arrInklaster[ii].z].radius*2;
                            k := Control.grid[control.arrInklaster[ii].x,control.arrInklaster[ii].y,control.arrInklaster[ii].z].radius*2;
                            h := Control.grid[control.arrInklaster[ii].x,control.arrInklaster[ii].y,control.arrInklaster[ii].z].radius*2;
                            color[0] := (random(100) + 50) / 150.0;
                            color[1] := (random(100) + 50) / 150.0;
                            color[2] := (random(100) + 50) / 150.0;
