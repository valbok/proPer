...
uses
  ...
  ExtCtrls, ComCtrls, StdCtrls,  OpenGL, Menus, Splash, Buttons, Gauges,unControl;
...
type
  ...
  public
    ...
    objects : Array [0..maxobjects] of GLObject; // массив объектов
...
uses AddObject, ParSdvig, ParAngle, ParObject, unSets;
...
var 
        xi,yi,zi:integer;
        tmpc:TCOlor;
        ColSetMol:Integer;      //col установленный молекул
begin
        frmSets.ShowModal;
              ...
              for zi:=0 to lim_z-1 do
                for yi:=0 to lim_y-1 do
                 ...
                 if Control.grid[xi,yi,zi] <> nil then
                      ...
                      begin
                          x :=Control.grid[xi,yi,zi].x;
                          y :=Control.grid[xi,yi,zi].y;
                          z :=Control.grid[xi,yi,zi].z;
                          l := radius/2;
                          k := radius/2;
                          h := radius/2;
      ...
      //                    showmessage(intToStr(frmMain.objectcount)+#13#10+     intToStr(MaxObjects));
                          eColSets.Text:=IntToStr(ColSetMol);
                          eColSets.Update;
      //                    showmessage(intToStr(zi));
      //                  DrawScene (GL_RENDER);
                      end;
